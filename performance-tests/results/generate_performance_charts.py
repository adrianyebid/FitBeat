...
# ─────────────────────────────────────────────────────────────
# Stages defined in performance_test.js (case3)
# Each window is 30s; setup adds ~5s before t=0, so we use
# elapsed seconds from the first recorded timestamp.
# ─────────────────────────────────────────────────────────────
STAGES = [
...
def parse_case3_performance(filename):
    """Parse case3_performance.json and bucket http_req_duration / http_req_failed
    into the five load stages defined above."""
    ...
    with open(filename) as f:
        for line in f:
            ...
            # Skip setup group (register call)
            tags = data.get("tags", {})
...
# ─────────────────────────────────────────────────────────────
# CHART 1 — Results table
# ─────────────────────────────────────────────────────────────
def chart_table(rows, out_path):
    col_headers = [
        "Usuarios\nconcurrentes",
        "Avg\n(ms)",
        "p(90)\n(ms)",
    ...
    ax.set_title(
        "FitBeat — POST /api/auth/login — Resultados por nivel de carga (case3_performance)\n"
        "🟢 Estable  🟡 Degradado  🔴 Fallo",
        fontsize=13, fontweight="bold", pad=16,
    )

    patches = [
        mpatches.Patch(color="#E8F5E9", label="Estable (errores = 0, p95 < 1 s)"),
        mpatches.Patch(color="#FFF9C4", label="Degradado (p95 > 1 s)"),
...
# ─────────────────────────────────────────────────────────────
def chart_knee_curve(rows, out_path):
    ...
    for v, a, p in zip(vus, avgs, p95s):
    ...
    ax1.axvspan(0,   50,   alpha=0.04, color="#4CAF50")
    ax1.axvspan(50,  200,  alpha=0.04, color="#FF9800")
    ax1.axvspan(200, 2100, alpha=0.06, color="#F44336")

    ax1.set_xlim(-30, 2200)
    ymax = max(p95s) * 1.05 if max(p95s) > 0 else 5000
    ax1.set_ylim(-200, ymax)

    ax1.set_ylabel("Tiempo de respuesta (ms)", fontsize=12)
    ax1.set_title(
        "FitBeat — Tiempo de respuesta vs Usuarios concurrentes (case3_performance)\n"
...
# ─────────────────────────────────────────────────────────────
def main():
    src = os.path.join(RESULTS_DIR, "case3_performance.json")
    if not os.path.exists(src):
        print(f"ERROR: {src} not found")
        return

    print(f"Parsing {src}...")
    buckets = parse_case3_performance(src)
    ...
    chart_table(rows, os.path.join(RESULTS_DIR, "chart_performance_table.png"))

    print("Generating knee curve chart...")
    chart_knee_curve(rows, os.path.join(RESULTS_DIR, "chart_performance_knee_curve.png"))
