...
| Artifact | Traefik, KrakenD with load balancing, `user-service` replicas, Redis cache, and PostgreSQL. |
| Environment | Normal operation with 1 and 50 virtual users generated by k6, reusing the same test scripts and stages used in the baseline run. |
| Response | The request crosses the public edge and API Gateway, KrakenD distributes traffic across `user-service` replicas, token validation uses Redis cache, and PostgreSQL is used when data is not already cached. |
| Response measure | Error rate remains at `0.0%`; p(95) latency remains below 1 second for sustained normal load (50 VUs). |
...
| Artifact | Traefik -> KrakenD (round-robin LB) -> `user-service` replicas -> Redis/PostgreSQL request path. |
...
| Response | The system processes requests with lower contention in the synchronous auth path; saturation is delayed and failure grows progressively instead of an immediate collapse. |
| Response measure | With the new patterns, the knee moves to 500 VUs, where p(95) reaches `10033 ms` and errors rise to `35.9%`. |
...
- **Distribute load across replicas:** KrakenD balances authentication requests across multiple `user-service` instances to avoid single-instance saturation.
- **Reduce repeated expensive operations:** Redis stores frequently accessed auth data, reducing repeated lookups against PostgreSQL.
...
- **Load Balancer Pattern:** KrakenD distributes incoming login traffic across scaled replicas.
- **Cache Pattern (Redis):** token validation and frequently reused auth data are served from in-memory cache before reaching PostgreSQL.

#### Performance testing analysis and results
To preserve traceability of the laboratory evidence, this section presents the **original baseline analysis** first, and then the **repeated tests after introducing Load Balancer + Redis Cache**.

##### Baseline analysis (before Load Balancer + Redis)
Following the laboratory objective of identifying the knee of the performance curve, the initial validation used k6 against the synchronous authentication path:
...
![Performance baseline results by load level](./performance-tests/results/chart_table.png)
...
![Baseline knee of the performance curve](./performance-tests/results/chart_knee.png)

In the baseline run, the system is stable at 1 and 50 VUs, the knee appears at **200 VUs**, and from 500 VUs onward the endpoint saturates with near-timeout latency and total failure.

##### Repeated tests after implementing Load Balancer Pattern + Cache Pattern (Redis)
Using the same test scripts and load stages, the team repeated the performance tests after introducing request distribution in KrakenD and Redis-backed cache in the auth path:

```text
k6 -> Traefik :8090 -> KrakenD :8085 -> user-service replicas -> Redis -> PostgreSQL
```

The endpoint measured remained `POST /api/auth/login`. The method was preserved for comparability: in `setup()`, k6 registers a test user once; during execution, each VU repeatedly performs login and waits one second to simulate user think time.

![Performance results by load level after LB + Redis](./performance-tests/results/chart_performance_table1.png)

| Concurrent users | Avg (ms) | p(90) (ms) | p(95) (ms) | Max (ms) | Throughput (req/s) | Errors |
|---:|---:|---:|---:|---:|---:|---:|
| 1 | 345 | 581 | 1585 | 2925 | 0.7 | 0.0% |
| 50 | 171 | 288 | 342 | 520 | 16.5 | 0.0% |
| 200 | 1447 | 2671 | 3428 | 10039 | 36.1 | 0.2% |
| 500 | 4639 | 10025 | 10033 | 10183 | 46.7 | 35.9% |
| 2000 | 9440 | 10019 | 10023 | 10165 | 83.5 | 93.7% |

![Knee of the performance curve after LB + Redis](./performance-tests/results/chart_performance_knee_curve1.png)

##### Comparative analysis (baseline vs LB + Redis)
The repeated tests show that capacity is extended and failure is delayed:

| Load level | Baseline (before) | LB + Redis (after) | Main change |
|---|---|---|---|
| 200 VUs | p(95)=8936 ms, errors=15.5% | p(95)=3428 ms, errors=0.2% | Strong improvement in tail latency and error rate |
| 500 VUs | avg=9605 ms, errors=100.0%, throughput=32.9 req/s | avg=4639 ms, errors=35.9%, throughput=46.7 req/s | Stops being total failure; better latency and throughput |
| 2000 VUs | errors=100.0%, throughput=118.9 req/s | errors=93.7%, throughput=83.5 req/s | Still failure zone; slight error-rate gain but lower throughput |

From these results, the practical knee moves from around **200 VUs** (baseline) to around **500 VUs** (with LB + Redis). This indicates that combining traffic distribution (Load Balancer pattern) and repeated-read reduction (Redis Cache pattern) increases headroom before saturation, although tail latency near timeout remains a bottleneck beyond 500 VUs.
