...
- Se utilizó una arquitectura de microservicios donde cada componente es independiente. El Componente A (FastAPI) gestiona autenticación e identidad, el Componente B (Go) gestiona sesiones de entrenamiento y reproducción, el Componente C (ASP.NET 8) gestiona logros/gamificación, y el frontend (React) actúa como cliente. La comunicación entre componentes sigue el estilo REST para operaciones síncronas (login, creación de sesión, OAuth) y WebSocket para el control de reproducción en tiempo real. Cada servicio posee su propia base de datos (Database per Service): PostgreSQL para datos relacionales de usuarios, CouchDB para documentos de sesiones de entrenamiento y SQLite para logros, garantizando el aislamiento de datos entre microservicios. El despliegue está containerizado con Docker Compose, agrupando todos los servicios en una red interna compartida.
...
- Componente C — Achievements Service (ASP.NET 8): Evalúa reglas de gamificación al finalizar una sesión de entrenamiento, desbloquea insignias y expone endpoints para consultar catálogo y progreso de logros por usuario.
...
- SQLite: Base de datos del Componente C. Almacena catálogo de logros, sesiones evaluadas y badges desbloqueados por usuario.
