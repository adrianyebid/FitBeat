...
- `traefik` (`fb_gateway`): reverse proxy located at the public boundary. It receives HTTP/HTTPS traffic, terminates TLS, applies CORS middleware, and forwards API traffic only to KrakenD. It also routes the SSR frontend.
- `krakend` (`fb_api_gateway`): API Gateway and orchestration facade. It exposes an explicit endpoint contract and routes requests to the corresponding backend service.
- `rabbitmq` (`fb_rabbitmq`): asynchronous message broker used for event publication and consumption between backend services.

##### Logic-type components
- `user-service` (`component_a`, Python/FastAPI): manages users, local authentication, JWTs, Spotify OAuth, and internal contact/token endpoints.
...
- **Integration decomposition:** Spotify and email providers are modeled as external systems and accessed through service-owned integration logic.
...
##### Scenario 1 - Real-time playback control under concurrent training sessions
| Element | Description |
|---|---|
| Source | Concurrent active users in training sessions. |
| Stimulus | Users send WebSocket playback commands such as play, pause, next, and previous. |
| Artifact | `music-service`, WebSocket connector, Spotify integration, and gateway path. |
| Environment | Normal operation during peak training usage. |
| Response | The system processes playback commands concurrently and avoids blocking session creation, event publication, or UI feedback. |
| Response measure | Target latency for command handling is under 200 ms for normal local-network conditions, with no blocked request threads during concurrent WebSocket handling. |

##### Scenario 2 - Asynchronous event processing without blocking training sessions
| Element | Description |
|---|---|
| Source | `music-service` and other producers emitting business events. |
| Stimulus | A burst of training-session events is generated while users continue controlling playback. |
| Artifact | RabbitMQ, `achievements-service`, `notification-service`, and `event-processor`. |
| Environment | Normal operation with event consumers active; consumers may process at different speeds. |
| Response | Producers publish events to RabbitMQ and return to the user-facing workflow without waiting for all consumers to finish. |
| Response measure | Active training flows remain responsive while consumers process queued events asynchronously; queue depth can absorb temporary spikes. |

#### Applied architectural tactics
- **Introduce concurrency:** The Go-based `music-service` handles WebSocket connections and playback operations concurrently.
- **Use asynchronous processing:** RabbitMQ removes achievement, notification, and metric processing from the critical user path.
- **Maintain multiple copies / horizontal scalability:** Stateless services such as `user-service`, `music-service`, `achievements-service`, and `notification-service` can be replicated behind Traefik/KrakenD in a production deployment.
...
- **Use efficient communication paths:** WebSocket is used for repeated player commands instead of creating a new HTTP request for each interaction.
- **Queue requests during bursts:** RabbitMQ buffers event traffic when consumers are temporarily slower than producers.

#### Applied architectural patterns
- **Broker Pattern:** RabbitMQ decouples producers from consumers and improves resilience under event bursts.
- **Load Balancing Pattern:** Traefik/KrakenD can distribute traffic across replicated stateless services.
...
- **WebSocket communication pattern:** persistent connections reduce overhead for real-time player control.

#### Performance testing analysis and results
The performance validation focuses on two critical paths:

1. **Synchronous real-time path:** client -> Traefik -> KrakenD -> `music-service` WebSocket/HTTP session endpoints.
2. **Asynchronous event path:** `music-service` -> RabbitMQ -> `achievements-service`, `notification-service`, and `event-processor`.

Expected test design:
- Use a load generator such as k6 or JMeter to simulate concurrent session creation and playback commands.
- Measure request latency, WebSocket command round-trip time, error rate, and throughput.
- Monitor RabbitMQ queue depth, consumer acknowledgement rate, and processing delay.
- Compare active-session responsiveness with and without event bursts.

Preliminary architectural analysis:
- The user-facing training flow is protected from heavy background work because achievement evaluation, notifications, and metric aggregation are moved to RabbitMQ consumers.
- The `music-service` is the most latency-sensitive component because it owns playback and WebSocket control.
- The gateway/reverse-proxy chain adds one additional hop, but it centralizes routing and security controls.
- The main scalability risks are Spotify API dependency latency, WebSocket connection volume, RabbitMQ consumer lag, and database write contention in event-heavy scenarios.

Result interpretation criteria:
- Playback command latency should remain below the defined target during normal concurrent load.
- API errors should remain close to zero for valid requests.
- RabbitMQ queues may grow during bursts, but they must drain after the burst without blocking new training sessions.
- No single consumer should become a permanent bottleneck; slow consumers can be scaled independently.
