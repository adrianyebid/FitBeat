...
```
+----------------------+     +----------------------------+     +-----------------------------------+     +----------------------------------+
|        Fuente        |     |          Estímulo          |     |           Artefacto               |     |         Respuesta / Medición     |
|                      |     |                            |     |  (Ambiente: operación normal)     |     |                                  |
| Atacante externo que |     | Intenta invocar endpoints  |     |  Endpoints internos del           |     | KrakenD rechaza con HTTP 404.    |
| conoce la API interna| --> | internos no publicados     | --> |  user-service que no están        | --> | Traefik solo enruta a KrakenD.   |
| del user-service     |     | (/auth/internal/token/id,  |     |  registrados en krakend.json      |     | El atacante no recibe información |
|                      |     | /api/auth/internal/contact)|     |                                   |     | de la topología interna.         |
+----------------------+     +----------------------------+     +-----------------------------------+     +----------------------------------+
```
...
|          Fuente           |     |            Estímulo              |     |              Artefacto                   |     |       Respuesta / Medición         |
|                           |     |                                  |     |  (Ambiente: operación normal)            |     |                                    |
| Atacante que ha           |     | Manipula la configuración de     |     |  Microservicios de backend               |     | Traefik resuelve la regla pero     |
| comprometido la config    | --> | Traefik para añadir una ruta     | --> |  (component_a, music_service, etc.)      | --> | el intento de conexión falla:      |
| de Traefik (dynamic.yml)  |     | directa a component_a:8000       |     |  ubicados en api_gateway_net             |     | DNS no resuelve desde gateway_net. |
|                           |     |                                  |     |                                          |     | Lateral movement imposible.        |
+---------------------------+     +----------------------------------+     +------------------------------------------+     +------------------------------------+
```
